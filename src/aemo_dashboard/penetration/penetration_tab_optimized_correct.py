...
class PenetrationTab:
    ...
    def _apply_smoothing_30day(self, df: pd.DataFrame, value_col: str = 'total_generation_mw') -> pd.DataFrame:
        ...
        if self.smoothing_select.value == 'Moving Average':
            ...
            # First sample to daily (take noon values)
            df['date'] = df['settlementdate'].dt.date
            df['hour'] = df['settlementdate'].dt.hour
            df['dayofyear'] = df['settlementdate'].dt.dayofyear
            df['year'] = df['settlementdate'].dt.year
            ...
            # Apply LOESS on daily data
            date_numeric = pd.to_datetime(daily_data['settlementdate']).astype(np.int64) / 1e9
            
            # Fraction for ~30 days
            frac = min(0.1, max(0.02, 30 / len(daily_data)))
            
            smoothed = lowess(
                daily_data[value_col],
                date_numeric,
                frac=frac,
                it=0,
                return_sorted=False
            ...
            daily_data['mw_smoothed'] = smoothed
            return daily_data
        
        else:
            # Fallback: return unsmoothed daily samples
            df['date'] = df['settlementdate'].dt.date
            df['hour'] = df['settlementdate'].dt.hour
            df['dayofyear'] = df['settlementdate'].dt.dayofyear
            df['year'] = df['settlementdate'].dt.year
            ...
            daily_data['mw_smoothed'] = daily_data[value_col]
            return daily_data
