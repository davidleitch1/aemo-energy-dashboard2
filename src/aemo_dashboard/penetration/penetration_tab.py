...
def _make_empty_fig(title: str, height: int = 400) -> go.Figure:
    """Create a placeholder Plotly figure with a centered message."""
    fig = go.Figure()
    fig.add_annotation(
        text=title,
...
def _apply_flexoki_layout(
    ...
    """Apply the standard Flexoki theme layout to a Plotly figure."""
    fig.update_layout(
        autosize=True,
        height=height,
        ...
        xaxis=dict(
            title=dict(text=xlabel, font=dict(size=12, color=FLEXOKI_BLACK)),
            showgrid=False,
            tickfont=dict(size=10, color=FLEXOKI_BLACK),
            zeroline=False,
        ),
        yaxis=dict(
            title=dict(text=ylabel, font=dict(size=12, color=FLEXOKI_BLACK)),
            showgrid=False,
            tickfont=dict(size=10, color=FLEXOKI_BLACK),
            zeroline=False,
        ),
...
class PenetrationTab:
    ...
    def __init__(self):
        ...
        # Chart panes — Plotly
        self.vre_production_pane = pn.pane.Plotly(
            object=None,
            sizing_mode='stretch_width',
            height=500,
        )

        self.vre_by_fuel_pane = pn.pane.Plotly(
            object=None,
            sizing_mode='stretch_width',
            height=500,
        )

        self.thermal_vs_renewables_pane = pn.pane.Plotly(
            object=None,
            sizing_mode='stretch_width',
            height=500,
        )
    ...
    def _show_error_plots(self, error_msg: str):
        ...
        error_fig = _make_empty_fig(f"Error loading data: {error_msg}", height=400)
        self.vre_production_pane.object = error_fig
        self.vre_by_fuel_pane.object = error_fig
        self.thermal_vs_renewables_pane.object = error_fig
    ...
    def _create_vre_production_chart(self):
        ...
        if df.empty:
            return _make_empty_fig(
                f'{self.fuel_select.value} production - No data available'
            )
        ...
        if df_filtered.empty:
            return _make_empty_fig(
                f'{self.fuel_select.value} production - No data'
            )
        ...
        # Cache the plot
        self._cache[plot_cache_key] = fig
    ...
    def _create_vre_by_fuel_chart(self):
        ...
        if df.empty:
            return _make_empty_fig('VRE production by fuel - No data available')

        # Filter for VRE fuels
        df_vre = df[df['fuel_type'].isin(['Wind', 'Solar', 'Rooftop'])].copy()

        if df_vre.empty:
            return _make_empty_fig('VRE production by fuel - No VRE data')
        ...
        if not has_traces:
            return _make_empty_fig('VRE production by fuel - No data to plot')
    ...
    def _create_thermal_vs_renewables_chart(self):
        ...
        if df.empty:
            return _make_empty_fig('Thermal v Renewables - No data available')
        ...
        if df_filtered.empty:
            return _make_empty_fig('Thermal v Renewables - No thermal/renewable data')
        ...
        fig.add_trace(go.Scatter(
            x=plot_data['settlementdate'],
            y=plot_data['thermal_twh'],
            mode='lines',
            name='coal+gas',
            line=dict(color=colors['coal+gas'], width=2),
        ))
