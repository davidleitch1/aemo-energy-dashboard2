        ...
        try:
            ...
            with perf_logger.timer("price_aggregation", threshold=0.5):
                ...
                if fuel_filters and 'fuel_type' in data.columns:
                    ...
                    logger.info("===== AFTER fuel filter: records=%d =====" % len(data))
                    if logger.isEnabledFor(logging.DEBUG):
                ...
                # DIAGNOSTIC: Show grouped data details before conversion
                if not grouped.empty:
                    logger.info("===== BEFORE CONVERSION: Groups=%d, Total_MW_sum=%.2f, Total_records=%d =====" % (len(grouped), grouped["total_generation_sum"].sum(), grouped["record_count"].sum()))
                # Calculate actual MWh generation based on resolution
                logger.info("========== DIAGNOSTIC: Resolution = %s ==========" % self.resolution)
                # scadavalue sum = sum of MW readings across intervals
                if self.resolution == '5min':
                    # Convert to MWh: sum(MW) × (5min / 60min/hr) = sum(MW) × (1/12) = MWh
                    logger.info("========== DIAGNOSTIC: Applied 5min factor (5/60) ==========")
                    grouped['generation_mwh'] = grouped['total_generation_sum'] * (5.0 / 60.0)
                    logger.info("===== AFTER 5MIN CONVERSION: Total_MWh=%.2f, Total_GWh=%.2f =====" % (grouped["generation_mwh"].sum(), grouped["generation_mwh"].sum()/1000))
                else:  # 30min
