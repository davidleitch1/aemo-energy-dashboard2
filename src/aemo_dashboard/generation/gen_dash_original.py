...
class EnergyDashboard(param.Parameterized):
    ...
    def load_generation_data(self):
        """Enhanced version using new generation adapter with intelligent resolution"""
        try:
            ...
            # Use enhanced adapter with auto resolution selection and performance optimization
            ...
            needs_optimization = days_span > 365  # Optimize for ranges > 1 year
            
            if needs_optimization:
                df, metadata = load_generation_data(
                ...
                logger.info(f"Performance optimization applied: {metadata.get('description', 'Unknown')}")
            ...
            logger.info(f"Loaded {len(df)} generation records using enhanced adapter with auto resolution")
