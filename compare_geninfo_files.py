...
def compare_duid_records(pickle_row: pd.Series, excel_row: pd.Series) -> Dict[str, Tuple[Any, Any]]:
    ...
    # Map pickle columns to excel columns
    field_mappings = {
...
def main():
    ...
    # Ensure DUID column exists and normalize
    if 'DUID' not in pickle_df.columns and pickle_df.index.name == 'DUID':
        pickle_df = pickle_df.reset_index()
    
    # Find common DUIDs
    pickle_duids = set(pickle_df['DUID'].str.strip().str.upper())
    excel_duids = set(excel_df['DUID'].str.strip().str.upper())
    ...
    for duid in sorted(common_duids):
        ...
        excel_row = excel_df[excel_df['DUID'].str.strip().str.upper() == duid].iloc[0]
